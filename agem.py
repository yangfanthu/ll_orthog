...
from model import EWCGaussianPolicy, QNetwork, EWCDeterministicPolicy, LLQNetwork
import random
import quadprog
import itertools
...
class AGEMSAC(object):
    ...
    def update_parameters(self, memory_list, batch_size, updates, previous_task_id=None):
        ...
        if self.task_id > 0 and self.args.learn_critic:
            ...
            avg_critic_grad = torch.mean(self.critic_grads[:,:self.task_id], dim=1)
            self.critic.zero_grad()
            ...
            current_task_critic_gradient = self.critic_grads[:, self.task_id]
            dotp = current_task_critic_gradient.dot(avg_critic_grad)
            if (dotp < 0).sum() != 0:
                updated_gradient = current_task_critic_gradient - (current_task_critic_gradient.dot(avg_critic_grad)/avg_critic_grad.dot(avg_critic_grad)) * avg_critic_grad
                # copy gradients back
                overwrite_grad(shared_critic_parameters, updated_gradient,
                                self.critic_grad_dims)
        ...
        if self.task_id > 0:
            ...
            avg_grad = torch.mean(self.grads[:,:self.task_id], dim=1)
            self.policy.zero_grad()
            ...
            current_task_gradient = self.grads[:, self.task_id]
            dotp = current_task_gradient.dot(avg_grad)
            if (dotp < 0).sum() != 0:
                updated_gradient = current_task_gradient - (current_task_gradient.dot(avg_grad)/avg_grad.dot(avg_grad)) * avg_grad
                # copy gradients back
                overwrite_grad(shared_parameters, updated_gradient,
                                self.grad_dims)
            # for param in shared_parameters:
...
if __name__ == "__main__":
    ...
    # Agent
    agent = AGEMSAC(env.observation_space.shape[0], env.action_space, num_tasks, args)
