...
from model import EWCGaussianPolicy, LLGaussianPolicy, QNetwork, DeterministicPolicy, CLOSDeterministicPolicy
import random


class SAC(object):
    def __init__(self, num_inputs, action_space, num_tasks, args, outdir):
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch = memory[self.task_id].sample(batch_size=batch_size)
    ...
    def save_model(self, suffix="", actor_path=None, critic_path=None):
        ...
        if critic_path is None:
            ...
            critic_target_path = os.path.join(self.outdir, "critic_target.ckpt".format(suffix))
...
class LLSAC(object):
    def __init__(self, num_inputs, action_space, num_tasks, args, outdir=None):
        ...
        if self.policy_type == "Gaussian":
            ...
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = CLOSDeterministicPolicy(num_inputs=num_inputs, 
                                           num_actions=action_space.shape[0], 
                                           hidden_dim=args.hidden_size,
                                           num_tasks=self.num_tasks,
                                           shared_feature_dim=args.shared_feature_dim,
                                           action_space=action_space).to(self.device)
            self.first_policy_optim = Adam(self.policy.parameters(), lr=args.lr)
            self.policy_optim = Adam(self.policy.mean_linears.parameters(), lr=args.lr)
